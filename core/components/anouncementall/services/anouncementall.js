 ...
 * @name $mmCourses
 */
.factory('$mmAnouncementall', function($q, $mmSite, $mmSitesManager, mmTimetableFrontPage) {

    var self = {},
        currentCourses = {};

    function storeAnouncementallInMemory(anouncementall) {
        angular.forEach(anouncementall, function(anouncementall) {
            currentAttendence[anouncementall.id] = anouncementall;
        });
    }
    ...
    self.getStoredCourse = function(id) {
        return currentAnouncementall[id];
    };
