...
.factory('$mmCourses', ["$q", "$mmSite", "$mmSitesManager", "mmCoursesFrontPage", function($q, $mmSite, $mmSitesManager, mmCoursesFrontPage) {
    ...
    function storeCoursesInMemory(courses) {
        ...
        return $mmSitesManager.getSite(siteid).then(function(site) {
            return site.read('local_user_courses_custom', data, presets).then(function(courses) {
...
.controller('mmAttendenceListCtrl', ["$scope", "$state","$mmAttendence","$ionicPlatform","$mmUtil", "$ionicHistory", function($scope, $state, $mmAttendence, $ionicPlatform, $mmUtil, $ionicHistory) {
	...
	fetchAttendence().finally(function() {
        $scope.attendenceLoaded = true;
    });
	 $scope.refreshAttendence = function() {
        fetchAttendence(true).finally(function() {
            $scope.$broadcast('scroll.refreshComplete');
        });
    };
...
angular.module('mm.core.attendence')
.factory('$mmAttendence', ["$q", "$mmSite", "$mmSitesManager", function($q, $mmSite, $mmSitesManager) {
    var self = {},
	currentAttendence = {};
         self.getUserAttendence = function(refresh, siteid) {
        siteid = siteid || $mmSite.getId();
        var userid = $mmSite.getUserId(),
            presets = {},
            data = {userid: userid};
        ...
        return $mmSitesManager.getSite(siteid).then(function(site) {
            return site.read('local_user_attendence_custom', data, presets).then(function(attendence) {
                //storeCoursesInMemory(attendence);
                return attendence;
            });
        });
...
.controller('mmTimetableListCtrl', ["$scope", "$mmTimetable","$ionicPlatform","$mmUtil","$ionicHistory","ionic", function($scope, $mmTimetable, $ionicPlatform,$mmUtil,$ionicHistory,ionic) {
	...
	fetchTimetable().finally(function() {
        $scope.timetableLoaded = true;
    });
	 $scope.refreshTimetable = function() {
        fetchTimetable(true).finally(function() {
            $scope.$broadcast('scroll.refreshComplete');
        });
    };
	 $ionicPlatform.registerBackButtonAction(function() {
...
angular.module('mm.core.timetable')
.factory('$mmTimetable', ["$q", "$mmSite", "$mmSitesManager", function($q, $mmSite, $mmSitesManager) {
    var self = {},
	currentTimetable = {};
         self.getUserTimetable = function(refresh, siteid) {
        siteid = siteid || $mmSite.getId();
        var userid = $mmSite.getUserId(),
            presets = {},
        ...
        return $mmSitesManager.getSite(siteid).then(function(site) {
            return site.read('local_user_todaystimetable_custom', data, presets).then(function(timetable) {
                //storeCoursesInMemory(attendence);
                return timetable;
            });
        });
...
angular.module('mm.core.tomtimetable')
.controller('mmTomtimetableListCtrl', ["$scope","$state", "$mmTomtimetable","$ionicPlatform","$mmUtil", "$ionicHistory", function($scope, $state, $mmTomtimetable,$ionicPlatform,$mmUtil, $ionicHistory) {
...
angular.module('mm.addons.mod_flexpaper')
    .factory('$mmaModFlexpaperCourseContentHandler', ["$mmCourse", "$mmaModFlexpaper", "$state", function($mmCourse, $mmaModFlexpaper, $state) {
        ...
        self.getController = function(module, courseid) {
            return function($scope) {
                $scope.icon = $mmCourse.getModuleIconSrc('flexpaper');
                $scope.title = module.name;
